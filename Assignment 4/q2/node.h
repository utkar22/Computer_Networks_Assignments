...
class Node {
 ...
 public:
  ...
  void sendMsg(){
    ...
    //We have to make sure that we are not sending a message to a node
    //that we already have sent a message to
    for (int i = 0; i<neighbours; i++){
      for (int j = 0; j<already_recv.size(); j++){
        if (already_recv[j]==interfaces[i].first.getConnectedIp()){
          return;
        }
      }
    }
    ...
    for (int i = 0; i < interfaces.size(); ++i) {
      ...
      msg.cost = interfaces[i].first.cost;
      msg.mytbl = &ntbl;
      msg.recvip = interfaces[i].first.getConnectedIp();
      already_recv.push_back(interfaces[i].first.getConnectedIp());		
